...
void fTest() {
	...
	string bin_string;
	cout << "denenecek deger dizisini giriniz (orn. 10100011010101110)" << endl;
	cout << "0 - geri don" << endl;
	cin >> bin_string;
	int number = 0;
	number = stoi(bin_string, 0, 2);
	// eger 0 girildiyse menuye don.
	if (number == 0) { return; }
	...
	unsigned long long counter = number; //tnum * thLimit
	int flag = 0;
	...
	while (!isSatisfiable && !isFinished) {
		...
		if (sonuc == 1) {
			isSatisfiable = true;
			cout << "verilen giris ile cozum saglandi, SAT" << endl;
			cout << "Solution: " << gBinary.substr(gBinary.length() - gVarNum, gVarNum) << endl;
		}
		// sonuc 0, ancak butun degerler denendiyse
		if (sonuc == 0) {
			cout << "verilen giris ile cozumu saglanmadi" << endl;
			break;
		}
		counter++;
